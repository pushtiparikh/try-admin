...
export default function ProductCard() {
  ...
  const addProductToCart = (item) => {
    ...
    if (currentData?.user?.length === 0 && !delivery_type) {
    ...
    if (!currentData.address && !delivery_type) {
    ...
    if (!currentData.deliveries && !delivery_type) {
      toast.warning(t('please.select.delivery'));
      return;
    }
    if (!currentData?.delivery_date && !delivery_type) {
      toast.warning(t('please.select.delivery.date'));
      return;
    }
    if (currentData?.delivery_time === null && !delivery_type) {
